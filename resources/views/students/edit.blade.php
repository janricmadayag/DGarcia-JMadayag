...
<head>
    ...
    <style>
        * { margin:0; padding:0; box-sizing:border-box; }
        ...
        .btn-update:hover { transform:translateY(-2px); box-shadow:0 5px 15px rgba(127,140,141,0.4); }
        .btn-cancel {
...
<body>
    <div class="form-container">
        ...
        <form action="{{ route('students.update', $student->id) }}" method="POST">
            ...
            <input type="text" name="contactNumber" value="{{ $student->contactNumber }}" required>
            ...
            <select name="section_id" required>
                @foreach($sections as $section)
                    <option value="{{ $section->id }}" {{ $student->section_id == $section->id ? 'selected' : '' }}>
                        {{ $section->course }} - Year {{ $section->year_level }} - Section {{ $section->section }}
                    </option>
                @endforeach
            </select>
